...
from vaybooks.bms.domain.production.activities.repository import ProductionActivityRepository
from vaybooks.bms.domain.shared.enums import ActivityCategory
from vaybooks.bms.domain.shared.exceptions import ValidationError


class ProductionActivityAppService:
    def __init__(self, activity_repo: ProductionActivityRepository):
    ...
    def _validate_config(self, config: ProductionActivityConfig) -> None:
    ...
    def list_activities(self, active_only: bool = True) -> List[ProductionActivityConfig]:
        return self._repo.list_all(active_only=active_only)

    def get_activity(self, activity_id: str) -> Optional[ProductionActivityConfig]:
    ...
    def create_activity(
        ...
        activity = ProductionActivityConfig(
            activity_name=name,
            activity_type=None,
    ...
    def deactivate_activity(self, activity_id: str) -> ProductionActivityConfig:
        activity = self._repo.find_by_id(activity_id)
        if not activity:
            raise ValueError("Error: Activity not found")
