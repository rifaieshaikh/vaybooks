...
import { useBusinessOverviewQuery } from '@vaybooks/store';
...
export function BusinessOverviewPage() {
  ...
  const { data, isLoading, error, refetch } = useBusinessOverviewQuery();
  ...
  return (
    <div>
      ...
      >
        <h2 style={{ margin: 0, color: 'var(--vb-color-primary, #185c4c)' }}>Business</h2>
        <Button type="button" variant="ghost" onClick={() => refetch()}>
          Refresh
        </Button>
      </div>
      {isLoading && <p>Loading…</p>}
      {error ? <ErrorText>Failed to load business overview.</ErrorText> : null}
      {!isLoading && !error && data && (
        <>
          ...
          <p style={{ color: '#667' }}>
            Activities: <Link to="/settings/business-activities">Business Activities</Link>
          </p>
        </>
      )}
    </div>
