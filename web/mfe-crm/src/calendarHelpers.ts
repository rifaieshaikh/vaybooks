...
export function activityToCalendarEvent(
  ...
  const durationMins = Number(row.duration_minutes);
  const durationLabel =
    Number.isFinite(durationMins) && durationMins > 0 ? `${Math.round(durationMins)} min` : '';
  const meta = [party, assignee, area, durationLabel].filter(Boolean).join(' · ');
  ...
  let end: string | undefined = endRaw
    ? endRaw.includes(' ')
      ? endRaw.replace(' ', 'T')
      : endRaw
    : undefined;
  if (!end && hasTime && Number.isFinite(durationMins) && durationMins > 0) {
    const startMs = new Date(start.replace(' ', 'T')).getTime();
    if (!Number.isNaN(startMs)) {
      end = new Date(startMs + durationMins * 60_000).toISOString();
    }
  }
  return {
    id,
