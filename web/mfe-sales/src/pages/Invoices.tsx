...
const DEFAULT_FILTERS = { store_invoice_number: '', customer_name: '', voucher_number: '' };
...
export function SalesInvoicesListPage() {
  ...
  return (
    <div>
      ...
      {!isLoading && !error && pageRows.length === 0 && <p>No invoices found.</p>}
      ...
      <EntityCardGrid>
        {pageRows.map((row) => {
          ...
          return (
            <EntityCard
              key={String(row.id)}
              title={asCaption(row.store_invoice_number) || asCaption(row.voucher_number) || String(row.id)}
              captions={[
                asCaption(row.customer_name || row.party_name),
                asCaption(row.sale_date || row.voucher_date).slice(0, 10),
                formatMoney(Number(row.net ?? row.gross ?? row.total ?? 0)),
                desc && desc.length <= 80 ? desc : '',
              ].filter(Boolean)}
              onView={() => navigate(`/sales/invoices/${row.id}`)}
            />
          );
        })}
      </EntityCardGrid>
